...
We need to select Vision Language Models (VLMs) to evaluate for identifying Stardew Valley inventory items in screenshots. The task requires a model to look at a grid of pixel-art game sprites and output structured data (item name, quantity) per cell.
...
- vLLM 0.7.x compatible on ROCm
- Apache 2.0 license
- Dynamic resolution: can process variable-size grids without fixed cropping

**SmolVLM2-2.2B-Instruct**:
- 3× smaller than Qwen2.5-VL — natural size/speed contrast for the talk
- Compresses each image to only **81 tokens** via SigLIP (vs. Qwen's dynamic thousands) — fast inference, but hypothesis: may lose fine-grained sprite detail
...
- CIA-Exact ≥ 50% zero-shot for at least one candidate (confirms the task is learnable)
